...
public class Farm_Old : MonoBehaviour
{
    public Grid grid;

    [SerializeField]
    private GameObject FertileLand;
    public List<FertileLand_Old> lands;
    ...
    void Awake()
    {
        ...
        lands.Add(Instantiate(FertileLand, new Vector3(transform.position.x - 1.0f, transform.position.y + 1.0f), Quaternion.identity, transform).GetComponent<FertileLand_Old>());
        lands[0].gameObject.name = FertileLand.name.Insert(0, "UL");
        lands.Add(Instantiate(FertileLand, new Vector3(transform.position.x, transform.position.y + 1.0f), Quaternion.identity, transform).GetComponent<FertileLand_Old>());
        lands[1].gameObject.name = FertileLand.name.Insert(0, "UC");
        lands.Add(Instantiate(FertileLand, new Vector3(transform.position.x + 1.0f, transform.position.y + 1.0f), Quaternion.identity, transform).GetComponent<FertileLand_Old>());
        lands[2].gameObject.name = FertileLand.name.Insert(0, "UR");
        lands.Add(Instantiate(FertileLand, new Vector3(transform.position.x - 1.0f, transform.position.y), Quaternion.identity, transform).GetComponent<FertileLand_Old>());
